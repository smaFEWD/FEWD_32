...
function renderObjectsFromArray() {
  ...
  // renderNewCard(movies[0].image, movies[0].cast, movies[0].teaser);
}
...
function renderMoviesAPI() {
  $.get('https://ga-movies-lite.firebaseio.com/movies.json', function(dataJSON) {
    // console.log(dataJSON[i]);
    for (var i = 0; i < dataJSON.length; i++) {
      renderNewCard(dataJSON[i].image, dataJSON[i].cast, dataJSON[i].teaser);
    };
  });
}
